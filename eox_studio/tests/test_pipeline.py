...
from eox_studio.edxapp_wrapper.users import get_user_signup_source
from eox_studio.pipeline import (
...
class EnsureUserPasswordUsableTest(TestCase):
    ...
    @patch('eox_studio.pipeline.get_user_attribute')
...
class EnsureUserProfileTest(TestCase):
    ...
    @patch('eox_studio.edxapp_wrapper.users.import_module')
...
class SignupSourceRegistration(TestCase):
    ...
    @patch("eox_studio.pipeline.UserSignupSource")
    @patch("eox_studio.pipeline.get_current_request")
