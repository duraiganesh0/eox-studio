...
class TesteGradeIntegration(TestCase):  # pragma: no cover
    ...
    @classmethod
    def setUpClass(cls):
        with open("eox_studio/tests/integration/test_data") as file_obj:
            cls.data = json.load(file_obj)
        cls.data["endpoint"] = "eox-studio/api/v1/grade/"
        site1_data = {
...
def create_enrollment(data):
    ...
    request_url = "{}/{}".format(
        data["site1_data"]["base_url"], "eox-studio/api/v1/enrollment/"
    )
