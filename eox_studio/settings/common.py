...
def plugin_settings(settings):
    """
    Defines eox-studio settings when app is used as a plugin to edx-platform.
    See: https://github.com/edx/edx-platform/blob/master/openedx/core/djangoapps/plugins/README.rst
    """
    # settings.EOX_CORE_COMMENTS_SERVICE_USERS_BACKEND = "eox_studio.edxapp_wrapper.backends.comments_service_users_j_v1"
    # settings.EOX_CORE_USERS_BACKEND = "eox_studio.edxapp_wrapper.backends.users_l_v1"
    # settings.EOX_CORE_ENROLLMENT_BACKEND = "eox_studio.edxapp_wrapper.backends.enrollment_l_v1"
    # settings.EOX_CORE_PRE_ENROLLMENT_BACKEND = "eox_studio.edxapp_wrapper.backends.pre_enrollment_l_v1"
    # settings.EOX_CORE_CERTIFICATES_BACKEND = "eox_studio.edxapp_wrapper.backends.certificates_h_v1"
    # settings.EOX_CORE_CONFIGURATION_HELPER_BACKEND = "eox_studio.edxapp_wrapper.backends.configuration_helpers_h_v1"
    # settings.EOX_CORE_COURSEWARE_BACKEND = "eox_studio.edxapp_wrapper.backends.courseware_h_v1"
    # settings.EOX_CORE_GRADES_BACKEND = "eox_studio.edxapp_wrapper.backends.grades_h_v1"
    # settings.EOX_CORE_STORAGES_BACKEND = "eox_studio.edxapp_wrapper.backends.storages_i_v1"
    # settings.EOX_CORE_ENABLE_STATICFILES_STORAGE = False
    # settings.EOX_CORE_STATICFILES_STORAGE = "eox_studio.storage.ProductionStorage"
    settings.EOX_CORE_LOAD_PERMISSIONS = True
    # settings.DATA_API_DEF_PAGE_SIZE = 1000
    # settings.DATA_API_MAX_PAGE_SIZE = 5000
    # settings.EDXMAKO_MODULE = "eox_studio.edxapp_wrapper.backends.edxmako_module"
    settings.EOX_CORE_COURSES_BACKEND = "eox_studio.edxapp_wrapper.backends.courses_l_v1"
    # # settings.EOX_CORE_COURSEKEY_BACKEND = "eox_studio.edxapp_wrapper.backends.coursekey_h_v1"
    # settings.EOX_CORE_SITE_CONFIGURATION = "eox_studio.edxapp_wrapper.backends.site_configuration_h_v1"
    ...
    settings.EOX_CORE_BEARER_AUTHENTICATION = 'eox_studio.edxapp_wrapper.backends.bearer_authentication_j_v1'
    # settings.EOX_CORE_ASYNC_TASKS = []
    # settings.EOX_CORE_THIRD_PARTY_AUTH_BACKEND = 'eox_studio.edxapp_wrapper.backends.third_party_auth_j_v1'
